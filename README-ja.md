...
- インストール済み翻訳キャッシュファイルの必須メタデータ、サイドカーメタデータの整合性、書/章/節構造、空でない節本文、決定的なチェックサムを検証します。
...
- ライブプロバイダーの HTTP 失敗では、有用なプロバイダーメッセージ、短い本文抜粋、利用可能な `Retry-After` の待機ヒント、一時的な失敗向けのリトライ案内を表示します。
- 未対応のライブプロバイダー構造では、欠落または不正なフィールドを示す実用的な診断を表示します。
- 手動リリース前に、ソースチェックアウトのパッケージングとドキュメントの準備状況を確認します。公開は行わず、レジストリインストールが利用可能だとも主張しません。
...
このワークフローをネットワークなしで確認するメンテナー向けスモークテストは、[docs/local-agent-smoke-transcript.md](docs/local-agent-smoke-transcript.md) を参照してください。`scripts/create_tiny_fixture_cache.py` で小さなローカルフィクスチャキャッシュを作成し、検証、マニフェスト生成、スキル本文生成、`John 3:16` の検索をライブプロバイダーなしで確認します。
...
`--data-dir` でダウンロード済み翻訳の保存先を指定します。指定しない場合、Bible Skill はプラットフォームに合ったユーザーデータディレクトリを使います。ダウンロード記録には、翻訳メタデータ、ソース URL、取得時刻、提供元が返すライセンス URL、そして正規化済み翻訳内容から計算した決定的な `sha256:` チェックサムが含まれます。取得時刻はチェックサムに含めません。`validate` コマンドは、Agent がキャッシュに依存する前に、インストール済みの `translation.json` とサイドカー `metadata.json` を検査します。任意の翻訳 ID を指定するとそのキャッシュだけを検証し、ID を省略するとすべてのインストール済み翻訳を検証します。サイドカーメタデータの欠落や不正な JSON、サイドカーのチェックサムドリフト、サイドカーメタデータと翻訳メタデータの不一致を報告します。テキスト出力は簡潔なタブ区切りで、`--json` は `translation_id`、`ok`、`checksum`、`issues` を含むオブジェクトを出力します。要求されたキャッシュのいずれかが存在しない、または無効な場合、検証は非ゼロで終了します。

ローカルキャッシュを別のマシンや Agent ワークスペースへ移す前に、`bible-skill cache manifest --data-dir ./data --json` を実行できます。このマニフェストは検査用の補助情報であり、パッケージやレジストリの主張ではありません。`schema_version`、`generated_at`、`data_dir` に加えて、各キャッシュディレクトリの id、名前、言語、ライセンス/ソース URL、書/章/節の数、チェックサム、POSIX 形式の `relative_path`、`validation_ok`、`issues` を出力します。自動化ではデータディレクトリをコピーし、転送前後のマニフェストを比較してから、転送先で `bible-skill validate --data-dir ./data` を実行できます。欠落または破損したキャッシュファイル、不正なサイドカーメタデータは、可能な限りマニフェスト全体を失敗させずに問題リストとして表現されます。検証で破損が報告された場合は、マニフェストを確認し、既定の dry-run として `bible-skill cache prune --data-dir ./data --json` を実行してから、確認後に `bible-skill cache prune TRANSLATION_ID --data-dir ./data --yes` を実行するか、ID を省略してすべての無効なキャッシュディレクトリを削除します。JSON prune 出力には `schema_version`、`dry_run`、`removed_count`、`removed`、`kept`、`issues` が含まれます。有効な項目は決して削除されず、要求された未存在 ID は問題として報告され、無関係な項目は削除されません。削除した翻訳は、検索する前に再ダウンロードしてください。`search` と `compare` コマンドはインストール済みのローカル翻訳だけを読むため、ローカルメタデータ検索や比較を行う各翻訳を先にダウンロードしてください。
...
ライブフォールバックでは、`--json` でプロバイダーの生レスポンスを出力でき、`--markdown` でノートに貼り付けやすい形式を出力でき、`--csv` で `reference`、`translation`、`verse_reference`、`text` 列を持つ表計算向けの行を出力できます。`--json`、`--markdown`、`--csv` は同時に指定できません。`--timeout SECONDS` で既定の 30 秒からプロバイダーリクエストのタイムアウトを変更でき、`--retries COUNT` で一時的なネットワークエラーや 408、429、5xx などの一時的な HTTP レスポンスを再試行できます。既定のリトライ回数は 0 のため、明示的に指定しない限り従来どおり 1 回だけ試行します。既定のリトライ回数でライブネットワーク失敗または一時的な HTTP プロバイダー失敗が起きた場合、CLI stderr は `--retries 2` を試すよう案内します。未対応のプロバイダー構造や無効な JSON の失敗では、この案内は表示しません。404/no passage found のような意味的なプロバイダー応答は再試行しません。Markdown と CSV のレンダリングは bible-api.com 形状のペイロードとの互換性を保ち、トップレベルの `data` オブジェクトで包まれたプロバイダーペイロード、`verses` または `passages` という名前の節リスト、`text`、`content`、`verse_text`、またはネストした配列/オブジェクトに格納された節本文にも対応します。ネストした断片は読みやすい空白で連結されます。未対応のライブ構造はレンダリング前に失敗し、`reference` の欠落、`data` の不正、`verses` または `passages` の不正、節エントリの不正、節本文の欠落を診断します。有効なプロバイダーペイロードでは、引き続き生の `--json` 出力を保持します。ライブプロバイダーが HTTP エラーを返した場合、CLI stderr にはステータス、読み取れるプロバイダーエラー項目または短く正規化されたプレーンテキスト本文、そして `Retry-After` 値があればそれも含まれます。
...
ソースチェックアウトでは `bible-skill release check` を使って、パッケージメタデータ、トップレベルの README/LICENSE ファイル、MIT ライセンス本文、`dist/` がある場合のビルド成果物メタデータ、README 変種内の未検証レジストリインストール主張を確認できます。これは公開前の準備状況チェックだけであり、何も公開せず、パッケージレジストリでの利用可能性も検証しません。`--json` は stdout に解析可能な自動化向け出力を返し、`--dist-dir PATH` で特定の成果物ディレクトリを確認できます。メンテナーは、これらのコマンドを [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) と組み合わせて使います。このチェックリストは、ソースチェックアウト検証、ビルド成果物レビュー、ドキュメントの正確性、明示的な未公開レジストリ状態を一か所にまとめています。
...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、キャッシュチェックサムとサイドカーメタデータの検証、キャッシュマニフェスト検査、無効キャッシュの prune、ローカル Markdown と USFM 出力、比較エクスポート、リリース準備状況チェック、文書化されたローカル agent スモークテスト transcript、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、タイムアウト/リトライ動作、リトライ案内、ライブ構造診断、ネットワーク/HTTP エラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。

## ロードマップ

Bible Skill は現在、保守プロジェクトとして管理し、通常は 2〜4 週間に 1 回、または提供元・キャッシュ・パッケージング・生成済みスキル同期に緊急の問題が出たときに対応します。完了レビュー、保守トリガー、運用ペースのルールは [ROADMAP.md](ROADMAP.md) を参照してください。

現在のロードマップの重点：

- 2026-05-21 の完了レビューで運用ペースを保守に下げました。通常作業では、検証済みのソースチェックアウト型リリース候補、README の正確性、リリースチェックリスト、生成済みスキル出力の同期を維持します。新しい成長フェーズの作業は、採用フィードバック、提供元の変更、または手動で承認・検証されたリリース経路が出るまで待ちます。
