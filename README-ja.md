...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、キャッシュチェックサムとサイドカーメタデータの検証、キャッシュマニフェスト検査、無効キャッシュの prune、ローカル Markdown と USFM 出力、比較エクスポート、リリース準備状況チェック、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、タイムアウト/リトライ動作、リトライ案内、ネットワーク/HTTP エラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。
...
- プロバイダーの schema 変更に対する診断をさらに詳しくし、ライブフォールバック失敗時にどの期待フィールドが欠落または不正だったかを示します。最初のソースチェックアウト版リリース候補は、ビルド成果物を使ったローカル確認に合格しましたが、パッケージレジストリ版はまだ公開も文書化もしていません。
