...
ソースチェックアウトでは `bible-skill release check` を使って、パッケージメタデータ、トップレベルの README/LICENSE ファイル、MIT ライセンス本文、`dist/` がある場合のビルド成果物メタデータ、README 変種内の未検証レジストリインストール主張を確認できます。これは公開前の準備状況チェックだけであり、何も公開せず、パッケージレジストリでの利用可能性も検証しません。`--json` は stdout に解析可能な自動化向け出力を返し、`--dist-dir PATH` で特定の成果物ディレクトリを確認できます。
...
- メンテナー向けのリリースチェックリストを準備し、実際のレジストリ公開が承認されるまで、ソースチェックアウト検証、ビルド成果物チェック、明示的な未公開レジストリ状態を一か所にまとめます。ライブプロバイダー構造診断は完了しています。
