...
- ローカル検索結果をノート向け Markdown または最小で決定的な USFM 風テキストとして出力します。
...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、キャッシュチェックサム検証、ローカル Markdown と USFM 出力、比較エクスポート、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、タイムアウト/リトライ動作とエラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。

## ロードマップ

- ローカル箇所エクスポートに、翻訳ライセンスとソース帰属の任意メタデータを追加します。
- オフライン/ローカル専用 Agent 環境向けのインストール案内を改善します。
