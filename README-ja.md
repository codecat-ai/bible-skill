...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、キャッシュチェックサムとサイドカーメタデータの検証、キャッシュマニフェスト検査、ローカル Markdown と USFM 出力、比較エクスポート、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、タイムアウト/リトライ動作とエラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。
