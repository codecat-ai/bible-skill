...
ローカルキャッシュを別のマシンや Agent ワークスペースへ移す前に、`bible-skill cache manifest --data-dir ./data --json` を実行できます。このマニフェストは検査用の補助情報であり、パッケージやレジストリの主張ではありません。`schema_version`、`generated_at`、`data_dir` に加えて、各キャッシュディレクトリの id、名前、言語、ライセンス/ソース URL、書/章/節の数、チェックサム、POSIX 形式の `relative_path`、`validation_ok`、`issues` を出力します。自動化ではデータディレクトリをコピーし、転送前後のマニフェストを比較してから、転送先で `bible-skill validate --data-dir ./data` を実行できます。欠落または破損したキャッシュファイル、不正なサイドカーメタデータは、可能な限りマニフェスト全体を失敗させずに問題リストとして表現されます。`search` と `compare` コマンドはインストール済みのローカル翻訳だけを読むため、ローカルメタデータ検索や比較を行う各翻訳を先にダウンロードしてください。
...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、キャッシュチェックサムとサイドカーメタデータの検証、キャッシュマニフェスト検査、ローカル Markdown と USFM 出力、比較エクスポート、リリース準備状況チェック、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、タイムアウト/リトライ動作、リトライ案内、ネットワーク/HTTP エラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。
...
- 無効なキャッシュ項目に対するローカルキャッシュ復旧ワークフローを修復し、文書化します。
