...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、USFM 出力と比較エクスポート、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイントとエラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。

## ロードマップ

- ダウンロード済み翻訳キャッシュファイルにチェックサムまたは schema 検証を追加します。
