...
- 2 つ以上のインストール済み翻訳で同じローカル箇所をテキスト、JSON、または Markdown で比較します。
- 翻訳全体をダウンロードせずに正確な箇所を調べるため、bible-api.com をライブフォールバックとして使えます。
...
`--data-dir` でダウンロード済み翻訳の保存先を指定します。指定しない場合、Bible Skill はプラットフォームに合ったユーザーデータディレクトリを使います。ダウンロード記録には、翻訳メタデータ、ソース URL、取得時刻、提供元が返すライセンス URL が含まれます。`compare` コマンドはインストール済みのローカル翻訳だけを読むため、比較する各翻訳を先にダウンロードしてください。
...
テストスイートは、参照解析、ローカル箇所検索と比較、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。

## ロードマップ

- より多くのプロバイダーデータ形状と USFM エクスポートに対応します。
- インストール済み翻訳のローカルメタデータ検索を追加します。
