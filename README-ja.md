...
テストスイートは、参照解析、ローカルメタデータ検索、ローカル箇所検索、キャッシュチェックサムとサイドカーメタデータの検証、キャッシュマニフェスト検査、無効キャッシュの prune、ローカル Markdown と USFM 出力、比較エクスポート、リリース準備状況チェック、Free Use Bible API レスポンスの正規化、プロバイダーのエンドポイント、タイムアウト/リトライ動作、リトライ案内、ライブ構造診断、ネットワーク/HTTP エラーフィクスチャ、保存/ダウンロード動作、CLI 出力、生成されるスキル本文をカバーします。
...
- メンテナー向けのリリースチェックリストは完了しました。次の重点は、小さなフィクスチャキャッシュを使って、もっとも一般的なローカル優先 agent ワークフローのスモークテスト transcript を記録し、メンテナーがネットワークなしで UX 変更を確認できるようにすることです。
