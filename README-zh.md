...
测试套件覆盖引用解析、本地元数据搜索、本地经文查询、缓存校验和验证、缓存清单检查、本地 Markdown 与 USFM 导出、对比导出、Free Use Bible API 响应规范化、提供方端点和错误夹具、存储/下载行为、CLI 输出和生成的技能文本。
...
- 改进实时提供方韧性，并记录有界重试和错误行为。
- 在 Agent 依赖已安装本地经文前，加强缓存/导入验证。
- 准备打包发布就绪检查，但在人工验证前不声明任何注册表安装路径。
