...
- 在实时提供方结构不受支持时，报告可操作的缺失或格式错误字段诊断。
...
如需对这条工作流做无需联网的维护者烟雾测试，请参见 [docs/local-agent-smoke-transcript.md](docs/local-agent-smoke-transcript.md)。它使用 `scripts/create_tiny_fixture_cache.py` 构建极小本地夹具缓存，然后验证、生成清单、生成技能文本，并在不依赖实时提供方的情况下查询 `John 3:16`。
...
在源码检出目录中使用 `bible-skill release check` 检查打包元数据、顶层 README/LICENSE 文件、MIT 许可证文本、存在 `dist/` 时的构建产物元数据，以及 README 变体中未验证的注册表安装声明。它只是发布前就绪检查；不会发布任何内容，也不会验证包注册表可用性。使用 `--json` 可在 stdout 输出可解析的自动化结果，使用 `--dist-dir PATH` 可检查指定的产物目录。维护者应将这些命令与 [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) 配套使用；该清单把源码检出验证、构建产物复核、文档真实性和明确的未发布注册表状态集中在一处。
...
测试套件覆盖引用解析、本地元数据搜索、本地经文查询、缓存校验和及旁路元数据验证、缓存清单检查、无效缓存修剪、本地 Markdown 与 USFM 导出、对比导出、发布就绪检查、已记录的本地 agent 烟雾测试 transcript、Free Use Bible API 响应规范化、提供方端点、超时/重试行为、重试提示、实时结构诊断、网络/HTTP 错误夹具、存储/下载行为、CLI 输出和生成的技能文本。

## 路线图

Bible Skill 现在按维护项目跟踪，节奏为每 2-4 周一次，并在出现紧急的提供方、缓存、打包或生成技能同步问题时处理。完整的完成度复核、维护触发条件和节奏规则见 [ROADMAP.md](ROADMAP.md)。

当前路线图重点：

- 2026-05-21 完成度复核已将节奏降为维护。例行工作应保持已验证的源码检出发布候选流程、README 真实性、发布清单和生成技能输出同步；新的成长型工作应等待采用反馈、提供方变化，或经过人工批准并验证的发布路径。
