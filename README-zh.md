...
- 在未下载整本译本时，使用 bible-api.com 作为精确经文查询的实时后备，并支持文本、JSON 或 Markdown 输出。
...
实时后备支持用 `--json` 输出原始 bible-api.com 响应，也支持用 `--markdown` 输出适合粘贴到笔记中的文本。`--json` 与 `--markdown` 互斥。
