...
测试套件覆盖引用解析、本地元数据搜索、本地经文查询、缓存校验和及旁路元数据验证、缓存清单检查、本地 Markdown 与 USFM 导出、对比导出、发布就绪检查、Free Use Bible API 响应规范化、提供方端点、超时/重试行为、重试提示、网络/HTTP 错误夹具、存储/下载行为、CLI 输出和生成的技能文本。
