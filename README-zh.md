...
实时后备支持用 `--json` 输出原始提供方响应，用 `--markdown` 输出适合粘贴到笔记中的文本，也支持用 `--csv` 输出适合电子表格的行，列为 `reference`、`translation`、`verse_reference` 和 `text`。`--json`、`--markdown` 与 `--csv` 互斥。Markdown 和 CSV 渲染保持兼容 bible-api.com 形状的载荷，也容忍包在顶层 `data` 对象中的提供方载荷、名为 `verses` 或 `passages` 的经文列表，以及存放在 `text`、`content`、`verse_text` 或嵌套数组/对象中的经文文本。嵌套片段会用可读间距连接。
