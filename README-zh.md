...
- 将本地查询结果导出为最小、确定性的类 USFM 文本。
- 以文本、JSON、Markdown 或 CSV 对比两个或更多已安装译本中的同一段本地经文。
- 在未下载整本译本时，使用 bible-api.com 作为精确经文查询的实时后备，并支持文本、原始 JSON、Markdown 或 CSV 输出。
...
测试套件覆盖引用解析、本地元数据搜索、本地经文查询、缓存校验和验证、USFM 导出和对比导出、Free Use Bible API 响应规范化、提供方端点和错误夹具、存储/下载行为、CLI 输出和生成的技能文本。

## 路线图

- 为自动化环境添加可配置的实时提供方超时和重试设置。
- 在人工注册表验证后准备打包发布。
